...
const RoomDetail: React.FC = () => {
  ...
  const fetchRoomServices = async () => {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`http://localhost:8080/api/v1/room-services/room/${roomId}`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    const fetchActiveContracts = async () => {
      try {
        ...
        const activeContracts = contracts.filter((contract: any) => {
          return contract.roomId === room.roomId && contract.contractStatus === "Active";
        });
        ...
        const students: Student[] = await Promise.all(
          activeContracts.map(async (contract: any) => {
            const userResponse = await fetch(`http://localhost:8080/api/v1/user/manager/${contract.userId}`, {
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
              },
            ...
            if (userResponse.ok) {
              ...
              return {
                ...
                fullName: userData.result?.fullName || userData.fullName || "N/A",
                studentCode: userData.result?.studentCode || userData.studentCode || "N/A",
              };
            } else {
              return {
                userId: contract.userId,
  ...
  const handleDeleteService = async (roomServiceId: string) => {
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`http://localhost:8080/api/v1/room-services/delete/${roomServiceId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleAddService = async () => {
    ...
    const token = localStorage.getItem("token");
    try {
      const response = await fetch("http://localhost:8080/api/v1/room-services/add", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="room-detail-container">
      ...
      <div className="room-detail-card">
        ...
        <p>
          <strong>Trạng thái:</strong> {room.roomStatus === "empty_room" ? "Còn trống" : "Đã đầy"}
        </p>
        <p>
          ...
          <ul>
            {activeStudents.map((student) => (
              <li key={student.userId}>
                <span className="student-code">Mã SV: {student.studentCode}</span>{" "}
            ...
            {roomServices.map((service) => {
              ...
              return (
                <li key={service.roomServiceId}>
                  <strong>
                    {serviceInfo ? serviceInfo.serviceName : "Tên dịch vụ không xác định"}
                  </strong>{" "}
            ...
            <label>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const selected = utilityServices.find((s) => s.utilityServiceId === value);
                  if (selected && selected.status !== "Active") {
                    setMessageText("Dịch vụ này không khả dụng.");
                    return;
                  }
                  // Kiểm tra xem dịch vụ đã được thêm chưa
                  const exists = roomServices.some((rs) => rs.utilityServiceId === value);
                  if (exists) {
              ...
              >
                ...
                {utilityServices.map((service) => (
                  <option
                    ...
                    disabled={service.status !== "Active" || roomServices.some(rs => rs.utilityServiceId === service.utilityServiceId)}
                  >
                    {service.serviceName} - {Number(service.pricePerUnit).toLocaleString()} VNĐ{" "}
                    {service.status !== "Active" ? "(Inactive)" : roomServices.some(rs => rs.utilityServiceId === service.utilityServiceId) ? "(Đã thêm)" : ""}
                  </option>
                ))}
              </select>
            </label>
            {selectedServiceDetail && (
              <div className="service-detail">
                ...
                <p>
                  ...
                  {Number(selectedServiceDetail.pricePerUnit).toLocaleString()} VNĐ/ {selectedServiceDetail.calculationUnit}
                </p>
              </div>
            )}
            <p>
              <strong>Giá dịch vụ:</strong>{" "}
              {price > 0 ? Number(price).toLocaleString() + " VNĐ" : "Chưa xác định"}
            </p>
